...
%     Phase varying channel
    angle = linspace(0,pi/4,length(tx));
    chann = exp(1i * angle);
...
EbN0_db = 0:10; 
EbN0 = 10 .^ (EbN0_db/10);
theore_ber = qfunc(sqrt(2 * EbN0));


figure;
semilogy(EbN0_db, theore_ber, 'r-', 'LineWidth', 1);
hold on;
semilogy(EbN0_db, BER, 'b-', 'LineWidth', 1); 
legend('AWGN', 'AWGN + Phase-Varying')
xlabel('E_b/N_0 (dB)'); 
ylabel('Bit Error Rate');
grid on


b_demo = bhat(nr_training_bits+1:end);
err = (b_demo == b_data);
figure;
scatter(0:3:999, err(1:3:end),  'filled')
xlabel('Bit Number (e.g., 1 => the first bit)')
ylabel('LogicalValue: 1/0')
set(gca, 'YLim', [-0.5 1.5]); 
legend(['LogicalValue=1 => No bit error; ' ...
    'LogicalValue=0 => Bit error']);
grid on
% x = 0:3:999
% scatter(x(~err(1:3:end)), err(~err(1:3:end)), 'filled', 'r');
% 
% hold on;
% 
% scatter(x(err(1:3:end)), err(err(1:3:end)), 'filled', 'b');