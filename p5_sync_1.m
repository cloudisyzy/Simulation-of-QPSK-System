...
% Initialization
EbN0_db = -10:1:10;                     % Eb/N0 values to simulate (in dB)
...
for snr_point = 1:length(EbN0_db)
  ...
  for blk = 1:nr_blocks
    ...
    % Next block.
    k = t_samp - t_start + 1;
    k_count_awgn = k_count_awgn + k;
  end
  k_hist_awgn = [k_hist_awgn, k_count_awgn/nr_blocks];
  % Next Eb/No value.
end
...
%     tx = multipath(tx, 6);  
    ...
    k = t_samp - t_start + 1;
    k_count_2path = k_count_2path + k;
  end
  k_hist_2path = [k_hist_2path, k_count_2path/nr_blocks];
  % Next Eb/No value.
end
...
figure;
plot(EbN0_db, k_hist_awgn)
hold on 
plot(EbN0_db, k_hist_2path)
