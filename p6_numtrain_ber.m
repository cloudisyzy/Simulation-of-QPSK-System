...
list = [6,10,20,30,50,100];
% Loop over different values of Eb/No.

BER_hist = zeros(length(list), length(EbN0_db));
...
figure;
for row = 1:size(BER_hist, 1)
    current_row = BER_hist(row, :);
    semilogy(EbN0_db, current_row);
    hold on
end

for row = 1:numel(list) %
    legend_labels{row} = sprintf("Training bits = %d", list(row));
end

legend(legend_labels);
xlabel('E_b/N_0 (dB)'); 
ylabel('Bit Error Rate');
% legend(sprintf("Training bits = %d", list(row)))